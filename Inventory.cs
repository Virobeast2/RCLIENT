...
public class Inventory : IDLocal
{
    ...
    public int CanConsume(ItemDataBlock db, int useCount, System.Collections.Generic.List<int> storeToList)
    {
        Collection<InventoryItem> collection = this.collection;
    ...
    {
        return new <FindItems>c__Iterator3B<IItemT> { <>f__this = this, $PC = -2 };
    }
    ...
    {
        this.SetActiveItemManually(itemIndex, !(itemRepID != uLink.NetworkViewID.unassigned) ? null : uLink.NetworkView.Find(itemRepID).GetComponent<ItemRepresentation>());
    }
    ...
    protected bool InitializeThisFixedSizeInventory()
    {
        ...
        if (this._collection_made_)
        {
            ...
            UnityEngine.Debug.LogError("Some how this inventory was already inititalized to a different size. It will be reinitialized. the original off size was " + this._collection_.Capacity, this);
        }
    ...
    {
        return this.ItemCombinePredicted(NetEntityID.Get((UnityEngine.MonoBehaviour) toInventory), fromSlot, toSlot);
    }
    ...
    {
        return this.ItemMergePredicted(NetEntityID.Get((UnityEngine.MonoBehaviour) toInventory), fromSlot, toSlot);
    }
    ...
    {
        NetCull.RPC<byte, byte, bool>((UnityEngine.MonoBehaviour) this, "ITSM", uLink.RPCMode.Server, (byte) fromSlot, (byte) toSlot, tryCombine);
    }

    private void ItemMergeRPC(NetEntityID toInvID, int fromSlot, int toSlot, bool tryCombine)
    {
        NetCull.RPC<NetEntityID, byte, byte, bool>((UnityEngine.MonoBehaviour) this, "ITMG", uLink.RPCMode.Server, toInvID, (byte) fromSlot, (byte) toSlot, tryCombine);
    }
    ...
    {
        return this.ItemMovePredicted(NetEntityID.Get((UnityEngine.MonoBehaviour) toInventory), fromSlot, toSlot);
    }
    ...
    {
        NetCull.RPC<byte, byte>((UnityEngine.MonoBehaviour) this, "ISMV", uLink.RPCMode.Server, (byte) fromSlot, (byte) toSlot);
    }

    private void ItemMoveRPC(NetEntityID toInvID, int fromSlot, int toSlot)
    {
        NetCull.RPC<NetEntityID, byte, byte>((UnityEngine.MonoBehaviour) this, "ITMV", uLink.RPCMode.Server, toInvID, (byte) fromSlot, (byte) toSlot);
    }
    ...
    public bool NetworkItemAction(int slot, InventoryItem.MenuItem option)
    {
        ...
        if (networkView != null)
        {
            ...
            networkView.RPC("IACT", uLink.RPCMode.Server, args);
            return true;
        }
        return false;
    }
    ...
    private void OnNetSlotUpdate(Collection<InventoryItem> _collection, int slot, bool occupied, uLink.BitStream invdata)
    {
        if (occupied)
        {
            InventoryItem item;
    ...
    protected void OnNetUpdate(uLink.BitStream invdata)
    {
        ...
        else
        {
            ...
            catch (Exception exception)
            {
                ...
                UnityEngine.Debug.Log(string.Format("numItemsInUpdate = {0}, iterated pos = {1}", num3, num5), this);
            }
        }
    }
    ...
    {
        NetCull.RPC<byte>((UnityEngine.MonoBehaviour) this, "SVUC", uLink.RPCMode.Server, RPCInteger(cell));
    }

    public void RequestFullUpdate()
    {
        NetCull.RPC((UnityEngine.MonoBehaviour) this, "SVUF", uLink.RPCMode.Server);
    }
        ...
        if (this.GetItem(slotNumber, out item))
        {
            ...
            if (((uses > 1) && this.anyVacantSlots) && item.datablock.IsSplittable())
            {
                ...
                if (num3 > 0)
                {
                    uses -= num3;
                    item.SetUses(uses);
                    NetCull.RPC<byte>((UnityEngine.MonoBehaviour) this, "ITSP", uLink.RPCMode.Server, (byte) slotNumber);
                    return true;
                }
            }
        }
    ...
    [CompilerGenerated]
    private sealed class <FindItems>c__Iterator3B<IItemT> : IDisposable, IEnumerator, IEnumerable, IEnumerable<IItemT>, IEnumerator<IItemT> where IItemT: class, IInventoryItem
    {
        ...
        IEnumerator<IItemT> IEnumerable<IItemT>.GetEnumerator()
        {
            ...
            return new Inventory.<FindItems>c__Iterator3B<IItemT> { <>f__this = this.<>f__this };
        }
