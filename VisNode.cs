...
public class VisNode : IDLocal
{
    ...
    private System.Collections.Generic.List<VisNode> cleanList;
    [NonSerialized]
    ...
    private void OnDestroy()
    {
        ...
        {
            UnityEngine.Debug.LogError("DESTROYING IN GUARDED UPDATE! " + base.name, this);
        }
        this.Unregister();
        RemoveNow(this);
    }
    ...
    private void Register()
    {
        if (this.awake && !this.active)
        {
            ...
            if (manager == null)
            {
                System.Type[] components = new System.Type[] { typeof(VisManager) };
            ...
            if (!this.dataConstructed)
            {
                ...
                this.cleanList = new System.Collections.Generic.List<VisNode>();
    ...
    private static void RouteMessageHSet(ODBSet<VisNode> list, string msg, object arg)
    {
        if (list.any)
        {
            ...
            do
            {
                ...
                {
                    UnityEngine.Debug.LogError(exception, self);
                }
            }
            while (first.has);
        }
    ...
    private static void RouteMessageList(RecycleList<VisNode> list, string msg, object arg)
    {
        using (RecycleListIter<VisNode> iter = list.MakeIter())
        {
            while (iter.MoveNext())
            {
                ...
                {
                    UnityEngine.Debug.LogError(exception, iter.Current);
                    continue;
                }
            }
        }
