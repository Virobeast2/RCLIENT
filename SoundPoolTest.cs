...
public class SoundPoolTest : MonoBehaviour
{
    ...
    private void Update()
    {
        if ((this.clips != null) && (this.intervalPlayRandomClip > 0f))
        {
            ...
            {
                AudioClip clip = this.clips[UnityEngine.Random.Range(0, this.clips.Length)];
                if (((this.on != null) && (this.on.Length > 0)) && (UnityEngine.Random.value <= this.chanceOn))
                {
                    clip.Play(this.on[UnityEngine.Random.Range(0, this.on.Length)]);
                }
                else
                {
                    clip.Play();
