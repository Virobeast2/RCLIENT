    ...
    private void Update()
    {
        ...
        if (!this.wasMuted)
        {
            if (Time.time > this.nextMusicTime)
            {
                ...
                AudioClip clip = this.tracks[UnityEngine.Random.Range(0, this.tracks.Length)];
                base.audio.Stop();
                base.audio.clip = clip;
                this.nextMusicTime = (Time.time + clip.length) + (this.timeBetweenTracks * UnityEngine.Random.RandomRange((float) 0.75f, (float) 1.25f));
                base.audio.Play();
            }
