...
public class FireBarrel : LootableObject, IServerSaveable, IActivatable, IActivatableToggle, IContextRequestable, IContextRequestableMenu, IContextRequestableQuick, IContextRequestableText, IContextRequestablePointText, IComponentInterface<IActivatable, Facepunch.MonoBehaviour, Activatable>, IComponentInterface<IActivatable, Facepunch.MonoBehaviour>, IComponentInterface<IActivatable>, IComponentInterface<IContextRequestable, Facepunch.MonoBehaviour, Contextual>, IComponentInterface<IContextRequestable, Facepunch.MonoBehaviour>, IComponentInterface<IContextRequestable>
{
    ...
    {
        if (base._currentlyUsingPlayer == uLink.NetworkPlayer.unassigned)
        {
            return "Use";
        }
    ...
    {
        this._lightFlickerTarget = this._lightIntensityInitial * UnityEngine.Random.Range((float) 0.75f, (float) 1.25f);
    }
    ...
    public void Update()
    {
        if (this.isOn)
        {
            ...
            {
                this._lightPosTarget = this._lightPosInitial + ((Vector3) (UnityEngine.Random.insideUnitSphere * 10f));
    ...
    private sealed class <ContextQueryMenu_FireBarrel>c__IteratorB : IDisposable, IEnumerator, IEnumerable, IEnumerable<ContextActionPrototype>, IEnumerator<ContextActionPrototype>
    {
        ...
        public bool MoveNext()
        {
            ...
            {
                case 0:
                    if (!(this.<>f__this._currentlyUsingPlayer == uLink.NetworkPlayer.unassigned))
                    {
                        goto Label_00AD;
                    }
