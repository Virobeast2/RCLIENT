...
public class BulletWeaponDataBlock : WeaponDataBlock
{
    ...
    public virtual void DoWeaponEffects(Transform soundTransform, Vector3 startPos, Vector3 endPos, Socket muzzleSocket, bool firstPerson, Component hitComponent, bool allowBlood, ItemRepresentation itemRep)
    {
        ...
        Tracer component = ((GameObject) UnityEngine.Object.Instantiate(this.tracerPrefab, startPos, Quaternion.LookRotation(forward))).GetComponent<Tracer>();
        if (component != null)
        {
    ...
    {
        return (UnityEngine.Random.Range(base.damageMin, base.damageMax) * (!this.IsSilenced(itemRep) ? 1f : 0.8f));
    }
    ...
    public virtual void PlayFireSound(Transform soundTransform, bool firstPerson, ItemRepresentation itemRep)
    {
        ...
        fireSound.PlayLocal(soundTransform, Vector3.zero, 1f, UnityEngine.Random.Range((float) 0.92f, (float) 1.08f), this.GetFireSoundRangeMin(), this.GetFireSoundRangeMax() * (!flag ? 1f : 1.5f), !firstPerson ? 20 : 0);
        if ((!firstPerson && (num > farFireSoundRangeMin)) && !flag)
        {
            AudioClip farFireSound = this.GetFarFireSound(itemRep);
            if (farFireSound != null)
            {
                farFireSound.PlayLocal(soundTransform, Vector3.zero, 1f, UnityEngine.Random.Range((float) 0.9f, (float) 1.1f), 0f, this.GetFarFireSoundRangeMax(), 50);
            }
        }
    }
    ...
    protected void WriteHitInfo(uLink.BitStream sendStream, ref Ray ray, bool didHit, ref RaycastHit2 hit)
    {
        ...
        if (didHit)
        {
            ...
            {
                unassigned = NetEntityID.Get((UnityEngine.MonoBehaviour) id.idMain);
