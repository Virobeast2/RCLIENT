...
public class SpawnInArea : MonoBehaviour
{
    ...
    {
        Vector3 size = UnityEngine.Terrain.activeTerrain.terrainData.size;
        Vector3 origin = new Vector3();
        bool flag = false;
        while (!flag)
        {
            origin = UnityEngine.Terrain.activeTerrain.transform.position;
            ...
            if (this.SpawnMap != null)
            {
                ...
                if ((grayscale > 0f) && (UnityEngine.Random.Range((float) 0f, (float) 1f) < grayscale))
                {
                    flag = true;
                }
                else
        ...
        base.transform.Rotate(Vector3.up * UnityEngine.Random.Range(0, 360), Space.World);
    }
}
