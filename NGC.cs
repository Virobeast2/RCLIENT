...
public sealed class NGC : UnityEngine.MonoBehaviour
{
    ...
    private NGCView Add(byte[] data, int offset, int length, uLink.NetworkMessageInfo info)
    {
        ...
        NGCView view = (NGCView) UnityEngine.Object.Instantiate(prefab.prefab, vector, rotation);
        view.creation = info;
        view.innerID = num3;
        view.prefab = prefab;
        view.outer = this;
        ...
        else
        {
            ...
            view.initialData = new uLink.BitStream(buffer, false);
        }
    ...
    private static RPCName CallRPCName(NetworkFlags? flags, NGCView view, int messageID, ref IEnumerable<uLink.NetworkPlayer> targets)
    {
        return CallRPCName(flags, view, messageID);
    }

    private static RPCName CallRPCName(NetworkFlags? flags, NGCView view, int messageID, ref uLink.NetworkPlayer target)
    {
        return CallRPCName(flags, view, messageID);
    }

    private static RPCName CallRPCName(NetworkFlags? flags, NGCView view, int messageID, ref uLink.RPCMode mode)
    {
        return CallRPCName(flags, view, messageID);
    }
    ...
    private static IExecuter FindExecuter(System.Type[] argumentTypes)
    {
    ...
    [Obsolete("NO, Use net cull making sure the prefab name string you must use starts with ;", true)]
    public static UnityEngine.Object Instantiate(UnityEngine.Object obj, Vector3 position, Quaternion rotation)
    {
        return UnityEngine.Object.Instantiate(obj, position, rotation);
    }

    private Procedure Message(int id_msg, byte[] args, int argByteSize, uLink.NetworkMessageInfo info)
    {
        return this.Message((id_msg >> 0x10) & 0xffff, id_msg & 0xffff, args, argByteSize, info);
    }

    private Procedure Message(byte[] data, int offset, int length, uLink.NetworkMessageInfo info)
    {
        int num4;
        byte[] buffer;
        int startIndex = offset;
    ...
    private Procedure Message(int id, int msg, byte[] args, int argByteSize, uLink.NetworkMessageInfo info)
    {
    ...
    {
        System.Collections.Generic.List<NGCView> list = new System.Collections.Generic.List<NGCView>(this.views.Values);
    ...
    private void ShootRPC(RPCName rpc, uLink.NetworkPlayer target, byte[] data)
    {
    ...
    private void ShootRPC(RPCName rpc, uLink.RPCMode mode, byte[] data)
    {
        if (rpc.flags == NetworkFlags.Normal)
        {
    ...
    private void ShootRPC(RPCName rpc, IEnumerable<uLink.NetworkPlayer> targets, byte[] data)
    {
        if (rpc.flags == NetworkFlags.Normal)
        {
    ...
    private static uLink.NetworkView SpawnNGC_NetworkView(string prefabName, NetworkInstantiateArgs args, uLink.NetworkMessageInfo info)
    {
        ...
        System.Type[] components = new System.Type[] { typeof(NGC), typeof(NGCInternalView) };
    ...
    {
        public static readonly System.Collections.Generic.List<NGC> all = new System.Collections.Generic.List<NGC>();
    ...
    public sealed class Prefab
    {
        ...
        public sealed class Installation
        {
            ...
            private static readonly Dictionary<System.Type, Method[]> methodsForType = new Dictionary<System.Type, Method[]>();
            ...
            public static NGC.Prefab.Installation Create(NGCView view)
            {
                ...
                foreach (UnityEngine.MonoBehaviour behaviour in view.scripts)
                {
                    Method[] methodArray;
                    System.Type key = behaviour.GetType();
                    if (!methodsForType.TryGetValue(key, out methodArray))
                    {
                        System.Collections.Generic.List<Method> list2 = new System.Collections.Generic.List<Method>();
            ...
            public struct Method
            {
                ...
                public void Invoke(ref Delegate d, NGC.Procedure procedure, ushort scriptIndex)
                {
                    ...
                    UnityEngine.MonoBehaviour instance = procedure.view.scripts[scriptIndex];
                    switch ((this.flags & 7))
                    {
                        case 0:
                ...
                public static NGC.Prefab.Installation.Method Create(MethodInfo info)
                {
                    ...
                    {
                        System.Type type2;
                        if ((type2 = parameters[parameters.Length - 1].ParameterType) == typeof(uLink.NetworkMessageInfo))
                        {
                            length--;
                            if ((length > 0) && ((type2 = parameters[length - 1].ParameterType) == typeof(uLink.BitStream)))
                            {
                                length--;
                                num = (byte) (num | 3);
                            }
                        ...
                        else if (type2 == typeof(uLink.BitStream))
                        {
                            length--;
                            num = (byte) (num | 2);
                        }
                    }
                    System.Type[] argumentTypes = new System.Type[length];
        ...
        public static class Register
        {
            ...
            private static readonly System.Collections.Generic.List<NGC.Prefab> Prefabs = new System.Collections.Generic.List<NGC.Prefab>();
