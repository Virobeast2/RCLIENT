...
public class TreeExplosion : MonoBehaviour
{
    ...
    private void Explode()
    {
        ...
        {
            if (Vector3.Distance(Vector3.Scale(instance.position, terrainData.size) + UnityEngine.Terrain.activeTerrain.transform.position, base.transform.position) < this.BlastRange)
            {
                GameObject obj2 = UnityEngine.Object.Instantiate(this.DeadReplace, Vector3.Scale(instance.position, terrainData.size) + UnityEngine.Terrain.activeTerrain.transform.position, Quaternion.identity) as GameObject;
