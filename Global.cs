...
{
    [Help("Prints fps at said interval", "interval (seconds)"), Admin, User, ConsoleSystem.Client]
    public static float fpslog = -1f;
    [ConsoleSystem.Client, Help("When set to True, all console printing will go through Debug.Log", ""), User]
    ...
    [Help("Creates an error", ""), ConsoleSystem.Client]
    public static void create_error(ref ConsoleSystem.Arg arg)
    {
        Debug.LogError("this is an error");
    }

    [User, Help("Prints something to the debug output", "string output"), ConsoleSystem.Client, Admin]
    public static void echo(ref ConsoleSystem.Arg arg)
    {
        arg.ReplyWith(arg.ArgsStr);
    }

    [Admin, User, ConsoleSystem.Client, Help("Search for a command", "string Name")]
    ...
    [Admin, ConsoleSystem.Client, Help("Quits the game", "")]
