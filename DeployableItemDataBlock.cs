...
public class DeployableItemDataBlock : HeldItemDataBlock
{
    ...
    public void CheckPlacementResults(Ray ray, out Vector3 pos, out Quaternion rot, out TransCarrier carrier, out DeployPlaceResults results)
    {
        ...
        bool flag8 = !flag7 ? Facepunch.MeshBatch.MeshBatchPhysics.Raycast(ray, out hit, placeRange, -472317957, out flag6, out instance) : Facepunch.MeshBatch.MeshBatchPhysics.SphereCast(ray, this.minCastRadius, out hit, placeRange, -472317957, out flag6, out instance);
        Vector3 point = ray.GetPoint(placeRange);
        if (!flag8)
        {
            Vector3 origin = point;
            origin.y += 0.5f;
            flag4 = Facepunch.MeshBatch.MeshBatchPhysics.Raycast(origin, Vector3.down, out hit, 5f, -472317957, out flag6, out instance);
        }
        if (flag8 || flag4)
        {
            ...
            if (((this.carrierSphereCastMode != CarrierSphereCastMode.Allowed) && flag8) && (flag7 && !NonVariantSphereCast(ray, position)))
            {
                ...
                if (!(flag10 = Facepunch.MeshBatch.MeshBatchPhysics.Raycast(ray2, out hit2, num2, -472317957, out flag9, out instance2)))
                {
                    Vector3 vector8 = position;
                    vector8.y += 0.5f;
                    flag10 = Facepunch.MeshBatch.MeshBatchPhysics.Raycast(vector8, Vector3.down, out hit2, 5f, -472317957, out flag9, out instance2);
                }
                if (flag10)
                {
