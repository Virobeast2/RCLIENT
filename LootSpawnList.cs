...
public class LootSpawnList : ScriptableObject
{
    ...
    private void PopulateInventory_Recurse(ref RecursiveInventoryPopulateArgs args)
    {
        ...
        {
            length = UnityEngine.Random.Range(this.minPackagesToSpawn, this.maxPackagesToSpawn);
        }
        for (int i = 0; !args.inventoryExausted && (i < length); i++)
        {
            ...
            UnityEngine.Object obj2 = entry.obj;
            if (obj2 != null)
            {
                if (obj2 is ItemDataBlock)
                {
                    ItemDataBlock datablock = obj2 as ItemDataBlock;
                    if (!object.ReferenceEquals(args.inventory.AddItem(datablock, Inventory.Slot.Preference.Define(args.spawnCount, false, Inventory.Slot.KindFlags.Belt | Inventory.Slot.KindFlags.Default), UnityEngine.Random.Range(entry.amountMin, entry.amountMax + 1)), null))
                    {
                        args.spawnCount++;
