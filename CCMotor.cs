...
public sealed class CCMotor : IDRemote
{
    ...
    private void MoveFromCollision(Collision collision)
    {
        ...
        if (component != null)
        {
            ...
            UnityEngine.Debug.DrawLine(start, start + zero, Color.yellow, 60f);
            this.ApplyMovementDelta(ref zero, 0f);
            UnityEngine.Debug.DrawLine(start, this.tr.position, Color.green, 60f);
            this.BindCharacter();
        }
    }
    ...
    private static class Callbacks
    {
        ...
        private static bool OnHit(CCDesc.HitManager HitManager, ref CCDesc.Hit hit)
        {
            ...
            if (CCMotor.ccmotor_debug && !(hit.Collider is TerrainCollider))
            {
                ...
                UnityEngine.Debug.Log(string.Format("{{\"ccmotor\":{{\"hit\":{{\"point\":[{0},{1},{2}],\"normal\":[{3},{4},{5}]}},\"dir\":[{6},{7},{8}],\"move\":{9},\"obj\":{10}}}}}", args), hit.GameObject);
            }
            tag.OnHit(ref hit);
            return true;
        }
