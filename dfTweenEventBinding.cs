...
{
    [CompilerGenerated]
    private static Func<ParameterInfo, System.Type> <>f__am$cacheC;
    ...
    private Delegate createDynamicWrapper(object target, System.Type delegateType, ParameterInfo[] eventParams, MethodInfo eventHandler)
    {
        System.Type[] first = new System.Type[] { target.GetType() };
        if (<>f__am$cacheC == null)
        {
            <>f__am$cacheC = p => p.ParameterType;
        }
        System.Type[] parameterTypes = first.Concat<System.Type>(eventParams.Select<ParameterInfo, System.Type>(<>f__am$cacheC)).ToArray<System.Type>();
        DynamicMethod method = new DynamicMethod("DynamicEventWrapper_" + eventHandler.Name, typeof(void), parameterTypes);
        ILGenerator iLGenerator = method.GetILGenerator();
        iLGenerator.Emit(OpCodes.Ldarg_0);
        iLGenerator.EmitCall(OpCodes.Callvirt, eventHandler, System.Type.EmptyTypes);
    ...
    {
        <getField>c__AnonStorey5D storeyd = new <getField>c__AnonStorey5D {
            fieldName = fieldName
        };
        return type.GetAllFields().Where<FieldInfo>(new Func<FieldInfo, bool>(storeyd.<>m__33)).FirstOrDefault<FieldInfo>();
    }
    ...
    [CompilerGenerated]
    private sealed class <getField>c__AnonStorey5D
    {
