...
public class RustLoader : Facepunch.MonoBehaviour, IRustLoaderTasks
{
    ...
    private sealed class <Start>c__IteratorA : IDisposable, IEnumerator, IEnumerator<object>
    {
        ...
        public bool MoveNext()
        {
            ...
            switch (num)
            {
                ...
                case 1:
                    ...
                    catch (Exception exception2)
                    {
                        ...
                        UnityEngine.Debug.LogException(this.<e>__4);
                    }
                    finally
                    {
                        this.<>f__this.loader = null;
                    }
                    this.$current = UnityEngine.Resources.UnloadUnusedAssets();
                    this.$PC = 2;
                    goto Label_02FD;
            ...
            if (!string.IsNullOrEmpty(this.<loaderError>__2))
            {
                UnityEngine.Debug.LogError(this.<loaderError>__2);
            }
            else
            {
                this.<>f__this.loader = Facepunch.Load.Loader.CreateFromText(this.<loaderText>__0, this.<bundleDirectory>__1, new FileDispatch());
