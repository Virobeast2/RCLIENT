...
public class Tracer : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        float num = UnityEngine.Random.Range((float) 0.75f, (float) 1f);
        this.startScale = new Vector3(base.transform.localScale.x * num, base.transform.localScale.y * num, base.transform.localScale.z * UnityEngine.Random.Range((float) 0.5f, (float) 1f));
    ...
    private void Update()
    {
        ...
        if (!flag ? this.colliderToHit.Raycast(ray, out hitInfo, distance) : Physics2.Raycast2(ray, out invalid, this.speedPerSec * num, this.layerMask))
        {
            ...
            if (flag2)
            {
                this.impactSounds[UnityEngine.Random.Range(0, this.impactSounds.Length)].Play(point, 1f, 2f, 15f, 180);
                GameObject obj4 = UnityEngine.Object.Instantiate(this.decalPrefab, point + ((Vector3) (normal * UnityEngine.Random.Range((float) 0.01f, (float) 0.03f))), rotation * Quaternion.Euler(0f, 0f, (float) UnityEngine.Random.Range(-30, 30))) as GameObject;
                if (gameObject != null)
                {
                    obj4.transform.parent = gameObject.transform;
                }
                UnityEngine.Object.Destroy(obj4, 15f);
            }
        }
        else
        {
