...
public class CraterMaker : MonoBehaviour
{
    ...
    {
        return new <RealCreate>c__Iterator4D { position = position, radius = radius, depth = depth, noise = noise, <$>position = position, <$>radius = radius, <$>depth = depth, <$>noise = noise, <>f__this = this };
    }

    [CompilerGenerated]
    private sealed class <RealCreate>c__Iterator4D : IDisposable, IEnumerator, IEnumerator<object>
    {
                ...
                case 0:
                    ...
                    while (this.<i>__9 < (this.<width>__4 * 2))
                    {
                        ...
                        while (this.<j>__10 < (this.<width>__4 * 2))
                        {
                            ...
                            if (this.noise > 0f)
                            {
                                this.<mod>__11 += (((this.<mod>__11 * this.<heightscale>__8) * this.depth) * UnityEngine.Random.value) * this.noise;
                            }
